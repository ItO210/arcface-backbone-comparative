...
def load_bin(path, image_size):
    ...
    data_list = []
    for flip in [0, 1]:
        data = torch.empty((num_images, 3, H, W), dtype=torch.float32)
    ...
    for idx in range(num_images):
        ...
        data_list[0][idx].copy_(img_tensor)
        flipped = torch.flip(img_tensor, dims=[2])
        data_list[1][idx].copy_(flipped)
...
def test(data_set, backbone, batch_size, nfolds=10):
    ...
    embeddings_list = []
    time_consumed = 0.0
    device = next(backbone.parameters()).device if any(p.requires_grad for p in backbone.parameters()) else torch.device('cpu')
    ...
    for embed in embeddings_list:
        for i in range(embed.shape[0]):
            ...
            _xnorm += _norm
            _xnorm_cnt += 1
    _xnorm /= _xnorm_cnt if _xnorm_cnt > 0 else 1.0
...
def dumpR(data_set,
    ...
    print(f'dumped verification embeddings -> {outname}')
