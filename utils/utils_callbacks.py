...
class CallBackVerification:
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        for i, dataset in enumerate(self.ver_list):
            acc1, std1, acc2, std2, xnorm, embeddings_list = verification.test(dataset, backbone, 10, 10)
            logging.info('[%s][%d]XNorm: %f' % (self.ver_name_list[i], global_step, xnorm))
            logging.info('[%s][%d]Accuracy-Flip: %1.5f+-%1.5f' % (self.ver_name_list[i], global_step, acc2, std2))

            if self.summary_writer:
                self.summary_writer.add_scalar(tag=self.ver_name_list[i],
                                               scalar_value=acc2,
                ...
                if fp16:
                    msg = ("Speed %.2f samples/sec   Loss %.4f   LearningRate %.6f   Epoch: %d   Global Step: %d   "
                           "Fp16 Grad Scale: %2.f   Required: %1.f hours") % (
                               speed, loss.avg, learning_rate, epoch, global_step,
                               grad_scaler.get_scale(), time_for_end
                           )
                else:
                    msg = ("Speed %.2f samples/sec   Loss %.4f   LearningRate %.6f   Epoch: %d   Global Step: %d   "
                           "Required: %1.f hours") % (
                               speed, loss.avg, learning_rate, epoch, global_step, time_for_end
                           )
                logging.info(msg)
                loss.reset()
                self.tic = time.time()
